...
// right now. This drives the map markers when it is reachable.
...
// reading about" interest signal per tool, and (see below) as the real
// number behind the headquarters fallback layer when GDELT is down.
...
// FURTHER NOTE (confirmed live again later): GDELT's public endpoint returns
// a hard HTTP 429 for long stretches regardless of caller or request
// spacing, not just from Cloudflare's shared egress IPs, it is simply an
// overloaded free service. Since the whole point of this page is "no
// invented numbers," an empty GDELT response can't be papered over with
// made-up mention counts. Instead, buildOriginFallbackCountries() below
// drives the map from a small, hand-verified list of real AI tool
// headquarters (REALM_TOOL_ORIGINS, each fact checked against public
// sources), sized by the same live Wikipedia numbers already being fetched
// for the search-interest column. That keeps every marker, caption, and
// popover grounded in something real and checkable even during a GDELT
// outage, and the page copy says so plainly.
...
// Used for the Wikipedia pageviews "search interest" column, and to attach a
// real, live number to the headquarters fallback layer below when a tool
// headquartered somewhere also happens to be one of these tracked articles.
...
{ tool: "Mistral AI", article: "Mistral_AI" },
{ tool: "DeepL", article: "DeepL" },
{ tool: "Grammarly", article: "Grammarly" },
{ tool: "Canva", article: "Canva" },
{ tool: "Stability AI", article: "Stability_AI" },
{ tool: "Cohere", article: "Cohere" },
{ tool: "ElevenLabs", article: "ElevenLabs" },
{ tool: "Synthesia", article: "Synthesia_(company)" },
{ tool: "AI21 Labs", article: "AI21_Labs" },
{ tool: "Tabnine", article: "Tabnine" },
{ tool: "Lovable", article: "Lovable_(company)" },
{ tool: "Upstage", article: "Upstage_(company)" },
{ tool: "Sarvam AI", article: "Sarvam_AI" },
{ tool: "Kling AI", article: "Kling_AI" },
];

// Hand-verified real headquarters facts for a subset of tracked AI tools,
// checked against public sources (company blogs, Wikipedia, press coverage)
// rather than guessed. Used only when GDELT's live news feed is down, see
// buildOriginFallbackCountries() below. Every "hq" city here is where that
// organization is actually headquartered or, for Grammarly, where it keeps a
// major R&D office, not an invented location.
const REALM_TOOL_ORIGINS = [
{ tool: "ChatGPT", country: "United States", hq: "San Francisco", org: "OpenAI" },
{ tool: "Claude", country: "United States", hq: "San Francisco", org: "Anthropic" },
{ tool: "Gemini", country: "United States", hq: "Mountain View", org: "Google DeepMind" },
{ tool: "GitHub Copilot", country: "United States", hq: "Redmond", org: "Microsoft / GitHub" },
{ tool: "Grok", country: "United States", hq: "San Francisco", org: "xAI" },
{ tool: "Perplexity", country: "United States", hq: "San Francisco", org: "Perplexity AI" },
{ tool: "Midjourney", country: "United States", hq: "San Francisco", org: "Midjourney, Inc." },
{ tool: "Runway", country: "United States", hq: "New York", org: "Runway AI, Inc." },
{ tool: "Notion AI", country: "United States", hq: "San Francisco", org: "Notion Labs" },
{ tool: "DeepSeek", country: "China", hq: "Hangzhou", org: "DeepSeek" },
{ tool: "Kling AI", country: "China", hq: "Beijing", org: "Kuaishou" },
{ tool: "Mistral AI", country: "France", hq: "Paris", org: "Mistral AI" },
{ tool: "Stability AI", country: "United Kingdom", hq: "London", org: "Stability AI" },
{ tool: "ElevenLabs", country: "United Kingdom", hq: "London", org: "ElevenLabs" },
{ tool: "Synthesia", country: "United Kingdom", hq: "London", org: "Synthesia" },
{ tool: "DeepL", country: "Germany", hq: "Cologne", org: "DeepL SE" },
{ tool: "Cohere", country: "Canada", hq: "Toronto", org: "Cohere" },
{ tool: "AI21 Labs", country: "Israel", hq: "Tel Aviv", org: "AI21 Labs" },
{ tool: "Tabnine", country: "Israel", hq: "Tel Aviv", org: "Tabnine" },
{ tool: "Canva", country: "Australia", hq: "Sydney", org: "Canva" },
{ tool: "Lovable", country: "Sweden", hq: "Stockholm", org: "Lovable" },
{ tool: "Upstage", country: "South Korea", hq: "Yongin", org: "Upstage" },
{ tool: "Sarvam AI", country: "India", hq: "Bengaluru", org: "Sarvam AI" },
{ tool: "Grammarly", country: "Ukraine", hq: "Kyiv", org: "Grammarly (Kyiv R&D hub)" },
];

const GDELT_BASE = "https://api.gdeltproject.org/api/v2/doc/doc";
const MAX_REALM_FLAVOR_PER_RUN = 6; // caption+image generation is the expensive part
const MAX_REALM_FLAVOR_PER_RUN_FALLBACK = 20; // one-time larger batch to light up the fallback map fast
...
// every 5 seconds..." message, or a bare HTTP 429, instead of JSON when hit
// too fast or when the service is simply overloaded). One call plus a couple
// of spaced-out retries gives GDELT a fair chance without hammering it, and
...
const headers = { "User-Agent": "SolosGemsRealmBot/1.0 (+https://solosgems.com)" };

let json = null;
const delaysMs = [0, 4000, 9000];
for (const delay of delaysMs) {
if (delay) await sleep(delay);
json = await fetchJsonSafe(url, { headers });
if (json) break;
}
...
// Builds a per-country breakdown from REALM_TOOL_ORIGINS (real, verified
// headquarters facts) instead of live news volume. Used only when GDELT's
// feed comes back empty and there is no prior good run to carry forward.
// Sizing prefers a real, live Wikipedia 7-day view count for the country's
// most-viewed tracked tool; when no tracked tool from that country has a
// wiki number, it falls back to the (also real, not invented) count of
// tracked tools headquartered there.
function buildOriginFallbackCountries(wikiByTool) {
const byCountry = new Map();
for (const origin of REALM_TOOL_ORIGINS) {
if (!byCountry.has(origin.country)) byCountry.set(origin.country, []);
byCountry.get(origin.country).push(origin);
}

const results = [];
for (const [country, origins] of byCountry.entries()) {
let best = null;
for (const o of origins) {
const w = wikiByTool.get(o.tool);
if (w && (!best || w.views7d > best.views7d)) {
best = { tool: o.tool, views7d: w.views7d };
}
}
const orgNames = Array.from(new Set(origins.map((o) => o.org)));
const cities = Array.from(new Set(origins.map((o) => o.hq)));
const originFact =
origins.length === 1
? `${origins[0].org} is headquartered in ${origins[0].hq}.`
: `Home to ${orgNames.join(", ")}, based in ${cities.join(" and ")}.`;

results.push({
country,
value: best ? best.views7d : origins.length,
metricLabel: best
? `Live Wikipedia views (7-day) for ${best.tool}`
: "Tracked AI tools headquartered here",
hotTool: best ? best.tool : origins[0].tool,
originFact,
});
}
...
"You are a snarky, nerdy Dungeon Master narrating a real-world AI data point as a " +
"region on a fantasy campaign map. You will be given real, verified facts about one " +
"country or region's connection to AI tools, this may be live news coverage volume, " +
"live Wikipedia search-interest numbers, a verified company headquarters location, or " +
"a combination. Use ONLY the facts given to you, do not invent any statistic, date, " +
"or claim not present in the input. Respond with ONLY a JSON object, no markdown " +
...
'AI numbers or headquarters fact could mean, grounded only in the given facts", ' +
'"whyInteresting": "1 sentence on why this specific data point is interesting", ' +
'"coolFact": "1 short sentence restating or contextualizing one of the given facts in a ' +
'fun way, not a new invented fact"}';

async function generateRealmCaption(countryName, stats, env) {
if (!env.AI) return null;
const lines = [`Country or region: ${countryName}`];
if (stats.metricLabel && stats.metricValue != null) {
lines.push(`${stats.metricLabel}: ${stats.metricValue}`);
}
if (stats.rank) lines.push(`Rank among tracked countries/regions on this page: ${stats.rank}`);
if (stats.hotTool) lines.push(`Notable AI tool tied to this region: ${stats.hotTool}`);
...
// Deterministic, no-AI-call fallback caption. Used only when the model
// call fails or is skipped, so a country never shows an empty popover.
// Built from the same verified fields as the AI prompt, no invented facts.
function buildStaticCaptionFallback(countryName, stats) {
const dndCaption =
`Behold, the realm of ${countryName}` +
(stats.hotTool ? `, home turf of ${stats.hotTool}` : "") +
`, rolls initiative on the campaign map.`;
const meaningBits = [];
if (stats.metricLabel && stats.metricValue != null) {
meaningBits.push(`${stats.metricLabel}: ${stats.metricValue}.`);
}
if (stats.originFact) meaningBits.push(stats.originFact);
if (!meaningBits.length) meaningBits.push(`Tracked on this map for its real, verified AI activity.`);
return {
dndCaption: dndCaption.slice(0, 240),
meaning: meaningBits.join(" ").slice(0, 400),
whyInteresting: stats.rank
? `Currently ranked ${stats.rank} among tracked countries and regions on this page.`.slice(0, 240)
: "",
coolFact: stats.originFact ? String(stats.originFact).slice(0, 240) : "",
};
}
...
const [globalCountries, githubTrending, wikiInterest] = await Promise.all([
gatherGlobalCountryMentions(),
...
// than wipe the map to nothing, carry forward the last known-good GDELT
// country data so the page keeps showing real (if slightly stale) mentions
// until the next successful gather, as long as that previous run was itself
// GDELT-driven (not the headquarters fallback, which regenerates fresh
// below instead of "carrying forward" a static dataset).
let carriedOverCountries = false;
if (
topGlobal.length === 0 &&
prevPayload &&
Array.isArray(prevPayload.countries) &&
prevPayload.countries.length > 0 &&
!prevPayload.usingOriginFallback
) {
...
// Still nothing real to show (GDELT down and no prior good run to carry
// forward): fall back to verified AI tool headquarters, sized by real live
// Wikipedia numbers where available. See buildOriginFallbackCountries().
let usingOriginFallback = false;
if (topGlobal.length === 0) {
usingOriginFallback = true;
const wikiByTool = new Map(wikiInterest.map((w) => [w.tool, w]));
topGlobal = buildOriginFallbackCountries(wikiByTool);
}

let flavorBudget = usingOriginFallback ? MAX_REALM_FLAVOR_PER_RUN_FALLBACK : MAX_REALM_FLAVOR_PER_RUN;
const countries = [];
...
const stats = {
metricLabel: entry.metricLabel || "AI news mentions (3-day GDELT volume score)",
metricValue: entry.value,
rank: i + 1,
hotTool: entry.hotTool || null,
sampleHeadline: headlines[0] ? headlines[0].title : null,
originFact: entry.originFact || null,
};
...
// Workers AI occasionally drops a call under load (observed roughly 1 in
// 12 during real batch runs). Rather than leave a country with an empty
// popover, fall back to a simple, non-AI caption built from the same
// verified facts, so every marker always has something to click into.
if (!caption) {
caption = buildStaticCaptionFallback(entry.country, stats);
}
...
lat: meta.lat,
lon: meta.lon,
mentions: stats.metricValue,
metricLabel: stats.metricLabel,
source: usingOriginFallback ? "origin-registry" : "gdelt",
originFact: stats.originFact || null,
rank: stats.rank,
